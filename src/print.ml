...
| TList ty ->
    open_hovbox 2; between "[" "]" (fun _ -> print_ty ty); close_box ();
| TRef ty -> open_hovbox 2; print_string "ref "; print_ty ty; close_box ();
| TFun (t1, t2) ->
    open_hovbox 2;
    between "(" ")"
      (fun _ -> print_ty t1; print_space (); print_string "->";
      print_space (); print_ty t2);
    close_box ();
...
| FieldVar fieldvar -> print_fieldvar fieldvar
| Bot -> printf "_|_"

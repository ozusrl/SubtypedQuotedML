...
let inferty_and_print exp env =
  try
    ...
    Format.print_flush ();
    print_ty ty;
    (*print_endline (StagedTypes.show_type ty);*)
    Format.print_flush ();
    print_endline ""
...
let rec parse_and_eval_exprs ?(repl = false) lexbuf =
  try
    let exp = StagedParser.main StagedLexer.mytoken lexbuf in
    inferty_and_print exp [StagedTypes.stdenv_tyrec];
    ...
    (try
      ...
      inferty_and_print translation [StagedTypes.stdenv_tyrec];
      eval_and_print_w_err
        RecordEval.run
        translation
